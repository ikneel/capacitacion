...
public class PersonasService implements MetodosPersona{
	...
	public Respuesta mostrarCiudad() {
		...
		for(Personas p : listap) {
			for(Personas p2 : listap) {
				if(p2.getCiudad().equalsIgnoreCase(p.getCiudad())) {
					personas++;
				}
	...
	public Respuesta buscarApellidos(String apellidop, String apellidom) {
		...
		for(Personas p : listap) {
			if(p.getApellidoPaterno().equalsIgnoreCase(apellidop) && p.getApellidoMaterno().equalsIgnoreCase(apellidom)) {
				listap_aux.add(p);
			}
		}
		
		rs.setMensaje("Las personas con los apellido ingresados son:");
		rs.setSucces(true);
		rs.setObj(listap_aux);
		return rs;
	}
